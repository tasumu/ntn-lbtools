...
const formSchema = calculationRequestSchema.refine(
  ...
  {
    message: "Select at least one Earth Station",
    path: ["earth_station_tx_id"],
  },
);
...
export function CalculationForm({
  ...
  const downlinkBandwidth = useWatch({
    control,
  ...
  const waveformOptions = Array.from(
    new Set(["DVB_S2X", "5G_NR", ...availableWaveforms]),
  ).map((w) => ({
    value: w,
    label: w.replace("_", "-"),
  }));
      ...
      reset({
        ...
        runtime: {
          ...
          downlink: {
            ...
            ground_lat_deg:
              initialValues.runtime?.downlink?.ground_lat_deg ?? 0,
            ground_lon_deg:
              initialValues.runtime?.downlink?.ground_lon_deg ?? 0,
  ...
  const firstErrorMessage = (
    ...
    while (stack.length) {
      ...
      if (Array.isArray(node)) {
        node.forEach((v, idx) =>
          stack.push({ path: `${path}[${idx}]`, node: v }),
        );
      } else if (typeof node === "object") {
        Object.entries(node).forEach(([k, v]) =>
          stack.push({ path: path ? `${path}.${k}` : k, node: v }),
        );
      }
    }
    return null;
  ...
  const prepareData = (values: CalculationFormValues): CalculationRequest => {
    ...
    const normalizeIntermod = (
      block?: CalculationRequest["runtime"]["intermodulation"],
    ) => {
      if (!block) return undefined;
      const applied =
        block.applied ||
    ...
    if (
      prepared.transponder_type === transponderTypeSchema.enum.REGENERATIVE &&
      !prepared.modcod_table_id
    ) {
      prepared.modcod_table_id =
        prepared.uplink_modcod_table_id || prepared.downlink_modcod_table_id;
    }
    ...
    const overrides = {
      satellite: cleanObject(
        values.overrides?.satellite as Record<string, unknown> | undefined,
      ),
    };
        ...
        <Accordion multiple variant="separated" aria-label="Advanced settings">
          ...
          {transponderType === transponderTypeSchema.enum.TRANSPARENT && (
            <Accordion.Item value="intermodulation">
              <Accordion.Control>
                Intermodulation (Transparent transponder)
              </Accordion.Control>
              <Accordion.Panel>
