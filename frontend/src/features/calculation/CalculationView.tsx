...
const formatModcod = (
  ...
  const target =
    "uplink" in modcod || "downlink" in modcod
      ? direction
        ? (modcod as any)[direction]
        : (modcod as any).downlink || (modcod as any).uplink
      : modcod;
  if (!target) return "-";
...
function loadScenario(scenario: any): CalculationRequest | null {
...
export function CalculationView() {
  ...
  const satelliteOptions = (satellitesQuery.data ?? []).map((s: any) => ({
    value: s.id,
    label: s.name,
  }));
  const earthStationOptions = (earthStationsQuery.data ?? []).map((e: any) => ({
    value: e.id,
    label: e.name,
  }));
  ...
  const optionsErrorMessage = optionsErrors
  ...
  const modcodSummary =
    modcodSelected &&
    ("uplink" in (modcodSelected as any) ||
      "downlink" in (modcodSelected as any))
                  ...
                  <Stack gap="sm" pr="xs">
                    {(scenariosQuery.data ?? []).map((s) => (
                      <Card
                        ...
                        style={{
                          borderColor:
                            selectedScenarioId === s.id ? "#1c7ed6" : undefined,
                        }}
                      >
                        <Group
          ...
          <Flex gap="md" direction={{ base: "column", md: "row" }}>
            <Card shadow="sm" withBorder style={{ flex: 1 }}>
              ...
              <Stack gap={4} mt="sm">
                <Text size="sm">
                  Elevation:{" "}
                  {formatDeg(mutation.data.runtime_echo?.uplink?.elevation_deg)}{" "}
                  deg
                </Text>
                <Text size="sm">
                  Tx EIRP: {formatDb(mutation.data.results.uplink.eirp_dbw)} dBW
                </Text>
                <Text size="sm">
                  Rx G/T: {formatDb(mutation.data.results.uplink.gt_db_per_k)}{" "}
                  dB/K
                </Text>
                <Text size="sm">
                  Pointing Loss:{" "}
                  {formatDb(
                    mutation.data.results.uplink.antenna_pointing_loss_db,
                  )}{" "}
                  dB
                </Text>
                <Text size="sm">
                  FSPL: {formatDb(mutation.data.results.uplink.fspl_db)} dB
                </Text>
                <Text size="sm">
                  Rain loss:{" "}
                  {formatDb(mutation.data.results.uplink.rain_loss_db)} dB
                </Text>
                <Text size="sm">
                  Gas loss: {formatDb(mutation.data.results.uplink.gas_loss_db)}{" "}
                  dB
                </Text>
                <Text size="sm">
                  C/N0: {formatDb(mutation.data.results.uplink.cn0_dbhz)} dB-Hz
                  | C/N: {formatDb(mutation.data.results.uplink.cn_db)} dB
                </Text>
                <Text size="sm">
                  C/(N+I): {formatDb(mutation.data.results.uplink.cni_db)} dB |
                  C/IM: {formatDb(mutation.data.results.uplink.c_im_db)} dB (
                  {formatApplied(
                    mutation.data.results.uplink.interference_applied,
                  )}{" "}
                  /{" "}
                ...
                {transponderType === "REGENERATIVE" && (
                  <Text size="sm">
                    Link margin:{" "}
                    {formatDb(mutation.data.results.uplink.link_margin_db)} dB
                  </Text>
                )}
                {isDirectionalModcod && (
                  <Text size="sm">
                    ModCod: {formatModcod(modcodSelected, "uplink")}
                  </Text>
                )}
              </Stack>
            </Card>
            ...
            <Card shadow="sm" withBorder style={{ flex: 1 }}>
              ...
              <Stack gap={4} mt="sm">
                <Text size="sm">
                  Elevation:{" "}
                  {formatDeg(
                    mutation.data.runtime_echo?.downlink?.elevation_deg,
                  )}{" "}
                  deg
                </Text>
                <Text size="sm">
                  Tx EIRP: {formatDb(mutation.data.results.downlink.eirp_dbw)}{" "}
                  dBW
                </Text>
                <Text size="sm">
                  Rx G/T: {formatDb(mutation.data.results.downlink.gt_db_per_k)}{" "}
                  dB/K
                </Text>
                <Text size="sm">
                  Pointing Loss:{" "}
                  {formatDb(
                    mutation.data.results.downlink.antenna_pointing_loss_db,
                  )}{" "}
                  dB
                </Text>
                <Text size="sm">
                  FSPL: {formatDb(mutation.data.results.downlink.fspl_db)} dB
                </Text>
                <Text size="sm">
                  Rain loss:{" "}
                  {formatDb(mutation.data.results.downlink.rain_loss_db)} dB
                </Text>
                <Text size="sm">
                  Gas loss:{" "}
                  {formatDb(mutation.data.results.downlink.gas_loss_db)} dB
                </Text>
                <Text size="sm">
                  C/N0: {formatDb(mutation.data.results.downlink.cn0_dbhz)}{" "}
                  dB-Hz | C/N: {formatDb(mutation.data.results.downlink.cn_db)}{" "}
                  dB
                </Text>
                <Text size="sm">
                  C/(N+I): {formatDb(mutation.data.results.downlink.cni_db)} dB
                  | C/IM: {formatDb(mutation.data.results.downlink.c_im_db)} dB
                  (
                  {formatApplied(
                    mutation.data.results.downlink.interference_applied,
                  )}{" "}
                  /{" "}
                  {formatApplied(
                    mutation.data.results.downlink.intermod_applied,
                  )}
                  )
                </Text>
                {transponderType === "REGENERATIVE" && (
                  <Text size="sm">
                    Link margin:{" "}
                    {formatDb(mutation.data.results.downlink.link_margin_db)} dB
                  </Text>
                )}
                {isDirectionalModcod && (
                  <Text size="sm">
                    ModCod: {formatModcod(modcodSelected, "downlink")}
                  </Text>
                )}
              </Stack>
            </Card>
            ...
            <Card shadow="sm" withBorder style={{ flex: 0.8 }}>
              ...
              <Stack gap={6} mt="sm">
                ...
                <Text size="sm">
                  C/(N+I): {formatDb(mutation.data.results?.combined?.cni_db)}{" "}
                  dB | C/IM:{" "}
                ...
                {transponderType === "REGENERATIVE" && (
                  <>
                    ...
                    <Text size="sm">
                      Downlink margin:{" "}
                      {formatDb(mutation.data.results.downlink.link_margin_db)}{" "}
                      dB
                    </Text>
                  </>
                )}
                ...
                {(() => {
                  ...
                  const maxVal = Math.max(
                    20,
                    Math.max(ulClean, dlClean, totalClean) + 5,
                  );
                  ...
                  const renderBar = (
                    ...
                    const degPercent = Math.min(
                      100,
                      Math.max(0, (degraded / maxVal) * 100),
                    );
                    const cleanPercent = Math.min(
                      100,
                      Math.max(0, (degradation / maxVal) * 100),
                    );
