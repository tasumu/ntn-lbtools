...
class SatelliteBase(BaseModel):
    ...
    transponder_bandwidth_mhz: float | None = Field(
        default=None, description="Physical limit for validation against runtime signal bandwidth",
    )
...
class EarthStationBase(BaseModel):
    ...
    antenna_diameter_m: float | None = Field(
        default=None, description="Used for future beamwidth/interference calculations",
    )
    antenna_gain_tx_db: float | None = Field(
        default=None, description="Tx Antenna Gain; used for EIRP calculation",
    )
    antenna_gain_rx_db: float | None = Field(
        default=None, description="Rx Antenna Gain; used for G/T calculation",
    )
    noise_temperature_k: float | None = Field(
        default=None, description="System Noise Temperature (K); used for G/T calculation",
    )
    eirp_dbw: float | None = Field(
        default=None, description="Total radiated power; if null, computed from TX Power + Gain",
    )
    tx_power_dbw: float | None = Field(
        default=None, description="Amplifier output power; used with Gain to calculate EIRP",
    )
    gt_db_per_k: float | None = None
